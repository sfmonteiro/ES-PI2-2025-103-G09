...
import { getConnection } from "../database/connection"; // padronizado
...
export default {
  ...
  async getAll(userId: number): Promise<Turma[]> {
    let conn;
    try {
      ...
      const sql = `
        ...
        ORDER BY t.NOME
      `;
      const result = await conn.execute<Turma>(
        sql,
        { userId },
  ...
  async getById(id: number, userId: number): Promise<Turma | null> {
    let conn;
    try {
      conn = await getConnection();
      const sql = `
  ...
  // Cria nova turma
  async create(data: Turma, userId: number): Promise<Turma> {
    let conn;
    try {
      conn = await getConnection();
      ...
      const checkDisc = await conn.execute(
        ...
        `,
        { idDisc: data.ID_DISCIPLINA, userId },
        { outFormat: oracledb.OUT_FORMAT_OBJECT }
      );
      ...
      const r = await conn.execute(sql, binds, { autoCommit: false }) as unknown as {
        outBinds: { newId: number[] }
      };
  ...
  // Atualiza turma
  async update(id: number, data: Partial<Turma>, userId: number): Promise<Turma | null> {
    let conn;
    try {
      conn = await getConnection();

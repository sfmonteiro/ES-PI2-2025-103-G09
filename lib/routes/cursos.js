...
router.get("/", auth_1.requireAuth, async (req, res) => {
    ...
    `;
        const result = await conn.execute(sql, { owner: userId }, { outFormat: oracledb_1.default.OUT_FORMAT_OBJECT });
        // Agrupar por curso
        const map = new Map();
...
router.post("/", auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        const chk = await conn.execute(`SELECT 1 FROM INSTITUICAO WHERE ID_INSTITUICAO = :id AND OWNER_USER_ID = :uid`, { id: id_instituicao, uid: userId }, { outFormat: oracledb_1.default.OUT_FORMAT_OBJECT });
    ...
    `;
        const r = await conn.execute(insertCurso, {
            nome,
            inst: id_instituicao,
...
router.put("/:id", auth_1.requireAuth, async (req, res) => {
    ...
    try {
      ...
      `, { curso: cursoId, uid: userId }, { outFormat: oracledb_1.default.OUT_FORMAT_OBJECT });
        if (!chk.rows || chk.rows.length === 0) {
            return res.status(404).json({ ok: false, message: "Curso não pertence ao usuário" });
        }
        // Atualizar nome do curso
        await conn.execute(`UPDATE CURSO SET NOME = :nome WHERE CURSO_ID = :id`, { nome, id: cursoId }, { autoCommit: false });
        // Atualizar disciplinas
        if (Array.isArray(disciplinas)) {
            // Buscar disciplinas atuais
            const old = await conn.execute(`SELECT ID_DISCIPLINA FROM DISCIPLINAS WHERE CURSO_ID = :id`, { id: cursoId }, { outFormat: oracledb_1.default.OUT_FORMAT_OBJECT });
            const oldIds = new Set(old.rows?.map((r) => r.ID_DISCIPLINA));
            for (const d of disciplinas) {
            ...
            for (const id of oldIds) {
                await conn.execute(`DELETE FROM DISCIPLINAS WHERE ID_DISCIPLINA = :id`, { id });
            }
        }
        await conn.commit();
...
router.delete("/:id", auth_1.requireAuth, async (req, res) => {
    ...
    try {
      ...
      `, { id: cursoId, uid: userId }, { outFormat: oracledb_1.default.OUT_FORMAT_OBJECT });
        if (!chk.rows || chk.rows.length === 0) {
            return res.status(404).json({ ok: false, message: "Curso não pertence ao usuário" });
        }
        // Verificar disciplinas restantes
        const chkDisc = await conn.execute(`SELECT 1 FROM DISCIPLINAS WHERE CURSO_ID = :id AND ROWNUM = 1`, { id: cursoId }, { outFormat: oracledb_1.default.OUT_FORMAT_OBJECT });
        ...
        // Excluir curso
        await conn.execute(`DELETE FROM CURSO WHERE CURSO_ID = :id`, { id: cursoId }, { autoCommit: false });
        await conn.commit();
